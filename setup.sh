...
if command -v cursor &>/dev/null; then
  ...
  VSIX_DIR=$(mktemp -d)
  EXTENSION_TAG="$(gh release list --repo figma/ai-prototype-scaffold --exclude-pre-releases --json tagName --jq '[.[] | select(.tagName | startswith("extension-"))][0].tagName' 2>/dev/null)"
