...
import type { ConnectorEndpoint, GeometryNode, SceneNode } from '../../scene-graph/types';
...
export function resolveAllConnectorPoints(
  ...
  return config.points.map((def, i) => resolveConnectorPoint(sg, node, def, i));
}
...
export function snapToConnectionPoint(
  ...
  const nearest = findNearestConnectorPoint(sg, node, worldX, worldY);
  if (nearest && nearest.distance <= snapThreshold) {
    return nearest.point;
  }

  // Fall back to nearest edge point
  return findNearestEdgePoint(sg, node, worldX, worldY);
}
...
export function resolveEndpointPosition(
  ...
  const targetNode = sg.getNode(endpoint.nodeId);
  ...
  if (endpoint.type === 'connected') {
    ...
    return resolveConnectorPoint(sg, targetNode, pointDef, endpoint.pointIndex);
  }

  // edge type: resolve fractional position on node bounds
  const world = getWorldPosition(sg, targetNode);
