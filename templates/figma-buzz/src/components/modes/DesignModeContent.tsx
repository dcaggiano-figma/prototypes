...
import {
  ButtonPrimitive, Checkbox, FormattedInput, HiddenLabel, HiddenLegend, IconButton, Input, Label, ScrollContainer, SegmentedControl, Select,
...
export function DesignModeContent() {
  ...
  return (
    <>
      {/* Header */}
      <div className="border-b border-border flex items-center justify-between pl-3 pr-1 h-40px">
        <span className="text-bodyMdStrong text-text">Design</span>
        ...
        <PropertyRow columns="1fr auto">
          <Input.Group columns="1fr 52px">
            ...
            <OpacityInput
              ...
              onChange={() => {
                // Canvas background doesn't support opacity
              }}
            />
          </Input.Group>
...
function StrokeSection({ node }: { node: AppearanceNode }) {
  ...
  const addStroke = useCallback(() => {
    ...
    store.updateNode(node.id, { strokes: [...node.strokes, newPaint] });
  ...
  const handlePositionChange = useCallback(
    (value: string) => {
      const pos = value as StrokeAlign;
      store.updateNode(node.id, { strokeAlign: pos });
    },
    [store, node.id],
  );
