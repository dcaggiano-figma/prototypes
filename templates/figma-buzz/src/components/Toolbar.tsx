...
function getAllToolIds(moveTool: ToolConfig, mainTools: ToolConfig[]): string[] {
...
const MODE_ORDER: Mode[] = ['buzz', 'design'];

const MODE_OPTIONS = [
  { value: 'buzz', icon: Icon24Buzz, label: 'Buzz view' },
...
export function Toolbar({ activeMode, onModeChange, isActionsOpen, onActionsOpenChange }: ToolbarProps) {
  ...
  const widthsRef = useRef<Record<Mode, number>>({ buzz: 0, design: 0 });
  ...
  useEffect(() => {
    const allIds = getAllToolIds(moveTool, mainTools);
