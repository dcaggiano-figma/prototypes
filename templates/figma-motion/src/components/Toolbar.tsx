...
const MODE_ORDER: Mode[] = ['draw', 'design', 'animate', 'dev'];

const MODE_OPTIONS = [
  { value: 'draw', icon: Icon24Draw, label: 'Draw view' },
  { value: 'design', icon: Icon24Design, label: 'Design view' },
  { value: 'animate', icon: Icon24Motion, label: 'Animate view' },
...
export function Toolbar({ activeMode, onModeChange, isActionsOpen, onActionsOpenChange, onQuickAction }: ToolbarProps) {
  ...
  const rowRefs = useRef<Record<Mode, HTMLDivElement | null>>({ draw: null, design: null, dev: null, animate: null });
  const widthsRef = useRef<Record<Mode, number>>({ draw: 0, design: 0, dev: 0, animate: 0 });
  ...
  const handleModeChange = (value: string) => {
    if (value === 'draw' || value === 'design' || value === 'animate' || value === 'dev') {
      onModeChange(value);
    }
  };
