...
export interface ViewportAPI {
  ...
  /** Callback invoked when the user zooms via wheel/pinch (not programmatic) */
  onUserZoomRef: React.MutableRefObject<(() => void) | null>
  /** Set pan limits in world coordinates; null = unlimited */
  setPanLimits: (limits: { minWX: number; minWY: number; maxWX: number; maxWY: number } | null) => void
}
...
export function ViewportProvider({ children }: { children: React.ReactNode }) {
  ...
  const onUserZoomRef = useRef<(() => void) | null>(null);
  const panLimitsRef = useRef<{ minWX: number; minWY: number; maxWX: number; maxWY: number } | null>(null);
  const setPanLimits = useCallback((limits: { minWX: number; minWY: number; maxWX: number; maxWY: number } | null) => { panLimitsRef.current = limits; }, []);
  ...
  useEffect(() => {
    ...
    function onWheel(e: WheelEvent) {
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        let newOX = origin.x - e.deltaX;
        let newOY = origin.y - e.deltaY;

        // Clamp pan to limits if set
        const limits = panLimitsRef.current;
        if (limits) {
          const margin = 80;
          const viewW = rect.width;
          const viewH = rect.height;
          newOX = Math.max(-margin - limits.maxWX * scale, Math.min(viewW + margin - limits.minWX * scale, newOX));
          newOY = Math.max(-margin - limits.maxWY * scale, Math.min(viewH + margin - limits.minWY * scale, newOY));
        }

        setState({
          origin: { x: newOX, y: newOY },
          scale,
        });
      }
    }
  ...
  const api = useMemo<ViewportAPI>(
    ...
    [state, setState, worldToScreen, screenToWorld, transform, zoomPercent, setMaxScale, setMinScale, setPanEnabled, setZoomAnchor, setPanLimits],
  );
