...
const { ToolButton, QuickActions } = SharedToolbar;
...
function getAllToolIds(moveTool: ToolConfig, handTool: ToolConfig, mainTools: ToolConfig[]): string[] {
  ...
  return [...moveIds, handTool.id, ...mainIds];
}
...
const MODE_ORDER: Mode[] = ['slide', 'design'];

const MODE_OPTIONS = [
  { value: 'slide', icon: Icon24Slides, label: 'Slide view' },
...
export function Toolbar({ activeMode, onModeChange, isActionsOpen, onActionsOpenChange }: ToolbarProps) {
  ...
  const rowRefs = useRef<Record<Mode, HTMLDivElement | null>>({ slide: null, design: null });
  const widthsRef = useRef<Record<Mode, number>>({ slide: 0, design: 0 });
  ...
  const handTool = toolbarConfigs.hand[activeMode];
  ...
  useEffect(() => {
    const allIds = getAllToolIds(moveTool, handTool, mainTools);
    if (!allIds.includes(activeTool)) {
      setActiveTool(allIds[0]);
    }
  }, [activeMode, moveTool, handTool, mainTools, activeTool]);
        ...
        >
          ...
          >
            {MODE_ORDER.map((mode) => {
              ...
              const modeHand = toolbarConfigs.hand[mode];
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 p-8px">
                    ...
                    />
                    <ToolButton
                      key={`${mode}-${modeHand.id}`}
                      id={modeHand.id}
                      Icon={modeHand.Icon}
                      label={modeHand.label}
                      activeTool={activeTool}
                      onSelectTool={handleSelectTool}
                    />
                    {modeMain.map((tool) => (
